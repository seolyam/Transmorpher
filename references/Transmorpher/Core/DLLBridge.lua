...
-- The DLL keeps the protected set; we only send the current MEMBER GUIDs. Their
-- pets/summons are covered automatically (the DLL matches pet owner GUID against
-- this set), so out-of-range pet GUIDs don't matter.
-- ============================================================
...
-- Apply the CVar-based "extra FPS" toggles (chat bubbles, sound effects). Global.
function ns.ApplyHideCVars()
    ...
    pcall(SetCVar, "Sound_EnableSFX", s.hideOtherSounds and "0" or "1")
end
