...
-- ============================================================
-- Make a checkbox's clickable area exactly match its visible box.
-- 3.3.5a's UICheckButtonTemplate / ChatConfigCheckButtonTemplate draw their
-- box/check textures at the template's own (larger, sometimes off-centre) size,
-- so after SetSize(22,22) the visible box overhangs the smaller hit frame — you
-- click the box and nothing toggles ("the activate area is outside the box").
-- Re-anchoring every texture to fill the frame and zeroing the hit-rect insets
-- makes the visible box and the clickable area one and the same.
function ns.NormalizeCheckboxHitRect(cb)
    if not cb then return end
    local n = cb:GetNormalTexture()
    local p = cb:GetPushedTexture()
    local h = cb:GetHighlightTexture()
    local c = cb:GetCheckedTexture()
    local d = cb.GetDisabledCheckedTexture and cb:GetDisabledCheckedTexture()
    for _, tex in ipairs({ n, p, h, c, d }) do
        if tex then tex:ClearAllPoints(); tex:SetAllPoints(cb) end
    end
...
function ns.CreateCheckbox(parent, label, tooltip)
    ...
    cb:SetSize(22, 22)
    ns.NormalizeCheckboxHitRect(cb)
