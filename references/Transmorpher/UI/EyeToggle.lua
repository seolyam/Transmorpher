...
local function CreateEyeButton(slot, isSpecial)
    ...
    -- Re-derive the hidden state from the CURRENT saved state. Stored-once state
    -- went stale after sets/loadouts/restore, so the button showed the wrong icon
    -- and the next click did the wrong action. Call this whenever slots change.
    eyeBtn.RefreshState = function(self)
        if self.isSpecial and self.slotName == "Mount" then
            self.isHidden = (TransmorpherCharacterState and TransmorpherCharacterState.MountHidden) or false
        else
            local eqId = ns.slotToEquipSlotId[self.slotName]
            ...
            self.isHidden = (stateHidden or stateItem == -1 or slot.isHiddenSlot == true) and true or false
        end
        if self.UpdateVisuals then self:UpdateVisuals() end
    end
    eyeBtn:RefreshState()
...
-- Re-sync every eye button's hidden state from the current saved state. Call after
-- any operation that changes slot contents without going through the eye button
-- itself (set apply, loadout apply, restore, login sync) so the eye icons and their
-- click behaviour always match reality.
function ns.RefreshAllEyeButtons()
    if not mainFrame or not mainFrame.slots then return end
    for _, slotName in pairs(ns.slotOrder or {}) do
        local slot = mainFrame.slots[slotName]
        if slot and slot.eyeButton and slot.eyeButton.RefreshState then
            slot.eyeButton:RefreshState()
        end
    end
end
